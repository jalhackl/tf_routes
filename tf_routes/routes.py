...
def cycle_checks_nx_v2(G):
    ...
    for i in cdict:
        ...
        for el in edg:
            if (cdict[i] == 1):
                G[el[0]][el[1]]['weight'] = G[el[0]][el[1]]['weight'] - 35/degreedict[i]
            else:
                G[el[0]][el[1]]['weight'] = G[el[0]][el[1]]['weight'] - 35/degreedict[i] - cdict[i]*2/degreedict[i]

            #just check that all weights are positive (should always be the case)
            if (G[el[0]][el[1]]['weight'] < 0):
                G[el[0]][el[1]]['weight'] = 0.1
